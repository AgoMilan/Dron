# -*- coding: utf-8 -*-
"""
Created on Wed Oct  1 19:08:21 2025
    ...
    while True:
        ...
        if selected_id is None and tracks and frame_idx >= args.pause_frame:
            if not paused_for_selection:
                ...
                print(f"⏸ Pauza na framu {frame_idx} – klikni na dron (levý klik) nebo stiskni klávesu pro auto-výběr")
                cv2.imshow("tracking", cv2.resize(vis, (args.winsize, args.winsize)) if args.winsize > 0 else vis)
                key = cv2.waitKey(0)

                # Ruční výběr kliknutím
                ...
                # Pokud neklikneš → fallback auto-výběr
