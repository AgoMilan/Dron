# -*- coding: utf-8 -*-
"""
yolo_deepsort_online_vivotek_v2_per.py
----------------------------------
Verze pro připojení na Vivotek IP kameru (RTSP) + YOLOv8 + DeepSORT

Úpravy:
 - Detekuje pouze osoby ("person")
 - Zmenšený box na oblast obličeje (nastavitelný parametr --face_ratio)
 - Tenčí zelený rámeček (1 px)
 - Možnost zobrazit FPS a počet detekovaných osob
"""
...
def extract_detections_from_result(res, face_ratio=0.4):
    """Vrátí seznam detekcí (pouze osoby) s oříznutím na horní část těla."""
    dets = []
    ...
    for i, b in enumerate(xyxy):
        ...
        # 🔹 Oříznutí rámečku jen na obličej / horní část těla
        y2 = y1 + int((y2 - y1) * face_ratio)
...
# ===============================
def main():
    parser = argparse.ArgumentParser(description="YOLO + DeepSORT online pro Vivotek RTSP – pouze osoby")
    ...
    parser.add_argument("--imgsz", type=int, default=640, help="Velikost vstupního obrázku pro YOLO")
    ...
    parser.add_argument("--face_ratio", type=float, default=0.4,
                        help="Podíl výšky postavy pro výřez obličeje (0–1, např. 0.3 = jen obličej)")
    ...
    print("▶️ Běží sledování osob... (Q/Esc ukončí)")
    ...
    while True:
        ...
        dets = extract_detections_from_result(results[0], face_ratio=args.face_ratio) if results else []
        ...
        cv2.putText(frame, f"FPS: {fps_sm:.1f}  Osoby:{det_count}", (10, 20),
