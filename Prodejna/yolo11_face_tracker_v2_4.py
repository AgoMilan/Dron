# -*- coding: utf-8 -*-
"""
Created on Tue Oct 14 14:22:42 2025

@author: Milan
"""

# yolo11_face_tracker_v2_4.py
# -*- coding: utf-8 -*-
"""
YOLO11face + DeepSORT + FaceRecognitionManager (v2_4)
- čtvercové trackovací rámečky zmenšitelné parametrem --shrink
- okamžité mizení po ztrátě osoby
"""
...
def extract_face_detections(res, frame_w, frame_h):
    """Získá detekce obličejů z YOLO výstupu a přepočítá na souřadnice rámce."""
    dets = []
...
def main():
    parser = argparse.ArgumentParser(description="YOLO11 Face Tracker v2_4 (RTSP + Recognition)")
    ...
    parser.add_argument("--shrink", type=float, default=0.55, help="zmenšení čtverce (0–1)")
    ...
    # okno
    win_name = "YOLO11 Face Tracker v2_4"
    if args.display:
        cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(win_name, winsize, int(winsize * frame_h / frame_w))

    print("[INFO] Spuštěno. Q / Esc = konec, E = přidat neznámou osobu")
    ...
    try:
        while True:
            ...
            for tr in tracks:
                ...
                # výřez pro rozpoznávání (celý)
                label_text, score = frm.update_track(frame, tid, (x1, y1, x2, y2))

                # zmenšený čtverec pro vizuální rámeček
                cx, cy = (x1 + x2) / 2.0, (y1 + y2) / 2.0
                side = min((x2 - x1), (y2 - y1)) * args.shrink
                sx1, sy1, sx2, sy2 = cx - side / 2, cy - side / 2, cx + side / 2, cy + side / 2
            ...
            if key in (27, ord('q'), ord('Q')):
                ...
                if last_unknown_emb is not None and last_unknown_crop is not None:
                    ...
                    print("[ENROLL] Žádná neznámá osoba k přidání.")
            frm.cleanup()
