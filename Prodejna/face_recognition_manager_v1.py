# -*- coding: utf-8 -*-
"""
Created on Tue Oct 14 16:07:06 2025
...
- výpočet face embeddingů (facenet-pytorch),
  správa galerie (JSON), buffering embeddingů pro každý track a rozhodování
  známá / neznámá.
- Přidáno:
    - delete_person(name)  -> smaže z galerie a uloží
    - list_gallery()       -> vrátí seznam jmen v galerii
"""
...
class FaceRecognitionManager:
    def __init__(
        ...
        # galerie: dict name -> embedding (numpy array)
        self.gallery = self._load_gallery()

        # per-track buffers: track_id -> deque(embeddings)
        self.track_buffers = defaultdict(lambda: deque(maxlen=self.buffer_size))
        self.track_last_seen = {}

        print(f"[FRM] Inicializováno (device={self.device}, gallery={len(self.gallery)} položek)")
        ...
        if os.path.exists(self.gallery_path):
            try:
                with open(self.gallery_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                # převést seznamy na numpy array
    ...
    def enroll_person(self, name, emb):
        ...
        if emb is None:
            print("[FRM] Nelze enrollovat: embedding je None.")
            return
    ...
    def delete_person(self, name):
        """Smaže osobu z galerie, pokud existuje."""
        if name in self.gallery:
            del self.gallery[name]
            self.save_gallery()
            print(f"[FRM] Osoba '{name}' byla odstraněna z galerie.")
            return True
        else:
            print(f"[FRM] Osoba '{name}' nebyla nalezena v galerii.")
            return False

    def list_gallery(self):
        """Vrátí seznam jmen v galerii (seřazené)."""
        names = sorted(list(self.gallery.keys()))
        return names
    ...
    def crop_face(self, frame, box):
        """Box: [x1,y1,x2,y2] ve pixelech (původní rozlišení). Vrací crop BGR nebo None."""
        h, w = frame.shape[:2]
    ...
    def get_embedding(self, face_crop):
        """Vrací L2-normalizovaný embedding (numpy) nebo None."""
    ...
    def update_track(self, frame, track_id, box):
        ...
        if len(self.track_buffers[track_id]) >= self.decision_min_samples:
            try:
                ...
                print(f"[FRM] Chyba při průměrování bufferu: {e}")
